...
namespace Excise
{
    class DiManager
    {
        ...
        public static string AddJournalEntryCredit(SAPbobsCOM.Company _comp, string creditCode, string debitCode,
            double amount, int series, string reference, string code, DateTime DocDate, int BPLID = 235, string currency = "GEL")
        {
            ...
            if (i == 0)
            {
                string transId = Company.GetNewObjectKey();
                return transId;
            }
            else
            {
                string des = _comp.GetLastErrorDescription();
                SAPbouiCOM.Framework.Application.SBO_Application.SetStatusBarMessage(des, BoMessageTime.bmt_Short);
                return des;
            }
        }
    }
